...
headers = {
    ...
    'x-request-id': "c9554371-147f-169f-06b3-6bc496f1ea8c",
    'x-signature': "629946cd40a069be9f22c2617b130d4a127692f6",
    'referer': "https://wx.zsxq.com/dweb/",
    'accept-encoding': "gzip, deflate, br",
    'accept-language': "en,zh-CN;q=0.9,zh;q=0.8,zh-TW;q=0.7",
    ...
    ws_address=wss%3A//ws.zsxq.com/ws%3Fversion%3Dv1.10%26access_token%3DA9C94B2A-ED9A-48B4-FFEA-315227C236CD;\
    zsxq_access_token=A9C94B2A-ED9A-48B4-FFEA-315227C236CD;\
    sajssdk_2015_cross_new_user=1;\
    sensorsdata2015jssdkcross=%7B%22distinct_id%22%3A%2216cb3667144547-0ecc86e2402a85-4c312272-2073600-16cb36671454c1%22%2C%22%24device_id%22%3A%2216cb3667144547-0ecc86e2402a85-4c312272-2073600-16cb36671454c1%22%2C%22props%22%3A%7B%7D%7D;\
    avatar_url=https%3A//images.zsxq.com/FiyukGO6GIxoPTWXduxdPiyo1Uaf%3Fe%3D1906272000%26token%3DkIxbL07-8jAj8w1n4s9zv64FuZZNEATmlU_Vm6zD%3AuUX-1Jn66J3pkIWTCMIoQII79Jg%3D;"
...
def get_download_url(headers,group_id):
    query_current_time='2017-03-19T23:12:11.300+0800'
    ...
    while True:
        ...
        # 后移5天 根据数据量确定
        query_current_time = datetime.utcfromtimestamp(int(datetime.strptime(query_current_time.replace('T', ' ').split('.')[0],'%Y-%m-%d %H:%M:%S').timestamp()) + mv_days*24 * 60 * 60).strftime('%Y-%m-%dT%H:%M:%S') + '.%d+0800' % (random.randint(100, 999))
        url = "https://api.zsxq.com/v1.10/groups/" + group_id + "/files?count=20&end_time=" + quote(query_current_time)
        print(url)
        # 对URL发起get请求获取页面内容
...
def file_write(file_name,file_url,create_time,download_count,size,group_id):
    try:
        f = open('./%s/log.txt'%(str(group_id)),'a',encoding='utf-8')
...
if __name__ == "__main__":
    ...
    group_id = "1824528822"
    print("正在下载星球：")
    get_download_url(headers,str(group_id))
