            ...
            for _ in range(int(num)):
                ...
                for i in range(trs_len):
                    ...
                    if o_account == search_account \
                        ...
                        # if o_no in downed_order_no_list:
                        #     continue
                        # # """抓到数据立即下注"""
                        # driver_account.find_element(By.XPATH, '//*[@id="number"]').send_keys(o_code)
                        # driver_account.find_element(By.XPATH, '//*[@id="gold"]').send_keys(o_momey)
                        # sleep(0.1)
                        # driver_account.find_element(By.XPATH,'//*[@id="kuaida"]/div[2]/div/table[2]/tbody/tr/td[3]/input[1]').click()
                        # sleep(0.2)
                        # print(format_time(), "抓到数据立即下注:", o_code,o_momey)
                        # downed_order_no_list.append(o_no)
                """下一页"""
...
def useData():
    ...
    if list is not None and len(list) > 0:
        ...
        for down in list:
            ...
            # sleep(1)
        # np.savetext(format_filename() + ".csv", list)
