...
if os.path.exists(os.path.join('D:/app', 'path.txt')):
    with open(os.path.join('D:/app', 'path.txt'),'r',encoding='utf-8') as f:
        path_cxt=str.strip(f.read())
        if(path_cxt!=None):
            if not os.path.exists(path_cxt):
                os.mkdir(path_cxt)
            deskPath=path_cxt
print('写入目录是：', deskPath)
# 多线程
'''
                    ...
                    if e.get('status') == 0:
                        ...
                        for p in str.split(pageListStr,'-'):
                            ...
                            for e1 in jsonobj['list']:
                                ...
                                if str(code).find('球')!=-1:
                                    ...
                                    data_list4.append('%s=%s' % (str(code), str(money)))
                                    data_list_all.append('%s=%s' % (str(code), str(money)))
                                elif type_num == 5:  # 任五
                        ...
                        with open(os.path.join(deskPath,'任五.txt'), 'a+', encoding='utf-8') as f:
                            if len(data_list5) > 0:
                                ...
                                print(format_time(), threadName,'写入', len(data_list5))
                        with open(os.path.join(deskPath,'任四.txt'), 'a+', encoding='utf-8') as f:
                            if len(data_list4) > 0:
                                # os.chmod(os.path.join(deskPath, '任四.txt'), stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO)
                                f.write(',' + ','.join(data_list4))
                                print(format_time(), threadName,'写入', len(data_list4))
