    ...
    while True:
        try:
            if threadDic.get(threadName)!=None:
                ...
                # global data_list = []  # 任四
                # global data_list1 = []  # 任一
                # global data_list2 = []  # 任二
                # global data_list3 = []  # 任三
                # global data_list5 = []  # 任五
                global data_list_all  # 注单
                for e in l:
                    if e.get('status') == 0:
                        ...
                        for p in str.split(pageListStr,'-'):
                            ...
                            for e1 in jsonobj['list']:
                                ...
                                if str(code).find('球')!=-1:
                                    ...
                                    q.put('%s=%s' % (str(codeStr), str(money)))
                                else :
                                    q.put('%s=%s' % (str(code), str(money)))
                        e['status'] = 1
...
class Example(QMainWindow):
    ...
    def genma(self):
        ...
        data2FileThread( 1, "data2FileThread_Order",q).start()
        data2FileThread( 2, "data2FileThread_type").start()
