...
class VarClazz():
    ...
    order_num = 0
    max_data_id = 0
    # 新数据标志 当所有线程拉数完毕时设置为False  线程拉数过程中如果有新数据插入 转为True
    newDataFlag = False
    countList = []
    codeDic = {}
    # 配置文件数据
    ...
    while True:
        try:
            ...
            if VarClazz.threadDic[threadName]['status']==1:
                ...
                if VarClazz.newDataFlag==False  :
                    for p in range(threadID,int(VarClazz.totalNo/20),VarClazz.threadNo):
                        if VarClazz.newDataFlag or VarClazz.threadDic[threadName]['status']==0:
                            break
                        ...
                        # 判断是否有新单注
                        VarClazz.curTotalPage=jsonobj.get('totalPage')
                        if VarClazz.lastTotalPage<VarClazz.curTotalPage:
                            print(format_time(), '有新注单,totalPage=',str(jsonobj.get('totalPage')))
                            VarClazz.lastTotalPage=VarClazz.curTotalPage
                            VarClazz.newDataFlag=True
                            break
                        ...
                        for e1 in _dataList:
                            ...
                            curPageIds.add(data_id)
                            curOrderNo=int(soup.span.text[-13:])
                            if p==threadID:
                                lstOrderNo=curOrderNo
                            if data_id in lstPageIds :
                                print(format_time(), '有新注单')
                                VarClazz.newDataFlag = True
                                break
                            if curOrderNo>lstOrderNo:
                                print(format_time(), '有新注单')
                                VarClazz.newDataFlag = True
                                break
                            ...
                            if VarClazz.codeDic.get(data_id):
                                ...
                                # VarClazz.threadDic[threadName]={'finish_status':1}
                                break
                            else:
                            ...
                            lstOrderNo=curOrderNo
                        lstPageIds=curPageIds
                        curPageIds=set()
                    # VarClazz.threadDic[threadName] = {'finish_status': 1}
                else :
                    if VarClazz.threadDic[threadName]['status']==1:
                        endPageNo=int(VarClazz.curTotalPage/VarClazz.threadNo)*VarClazz.threadNo+int(threadID)
                        print(format_time(), '有新注单,totalPage=',str(jsonobj.get('totalPage')),'endPageNo:',endPageNo)
                        topEnd=False
                        tailEnd=False
                        for p in range(endPageNo,threadID, -VarClazz.threadNo):
                            print(format_time(), threadName, '有新注单,拉取数据,page:', p, 'len:', len(_dataList))
                            if len(_dataList) == 0 or tailEnd==True:
                                VarClazz.countList.append(totalCount)
                                break
                            totalCount = +len(_dataList)
                            # print(format_time(),'pullData',_dataList[0:2])
                            for e1 in _dataList:
                                ...
                                if VarClazz.codeDic.get(data_id):
                                    ...
                                    VarClazz.codeDic[data_id] = '%s=%s' % (str(code), str(money))
                        for p in range(threadID, endPageNo, VarClazz.threadNo):
                            print(format_time(), threadName, '有新注单,拉取数据,page:', p, 'len:', len(_dataList))
                            if len(_dataList) == 0 or topEnd==True:
                                VarClazz.countList.append(totalCount)
                                break
                            totalCount = +len(_dataList)
                            # print(format_time(),'pullData',_dataList[0:2])
                ...
                for key in VarClazz.codeDic.keys():
                    codeStr=VarClazz.codeDic.get(key)
                    code=str(codeStr).split('=')[0]
                    money=str(codeStr).split('=')[1]
                    type_num = len([i for i in range(len(code)) if str.upper(code[i]) != 'X'])
                    if type_num == 1:  # 任一
                        data_list1.append('%s=%s' % (str(code), str(money)))
                    elif type_num == 3:  # 任三
...
class Example(QMainWindow):
    ...
    def genma(self):
        ...
        for i in range(VarClazz.threadNo):
            VarClazz.threadDic["Thread-%s" % (str(i+1))]={'status':0,'finish_status':0}#finish_status 线程数据拉去完成状态 0没有 1 已完成
        ...
        while True:
            try:
                ...
                jsonobj = json.loads(response.text)
                VarClazz.lastTotalPage=jsonobj.get('totalPage')
