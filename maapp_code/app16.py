...
def pullData(threadName,threadID,driver_account):
    ...
    while True:
        try:
            #
            if threadDic[threadName]['status']==1:
                ...
                data_list_all = []  #
                totalCount=0
                for p in range(threadID,int(totalNo/200),threadNo):
                    ...
                    for e1 in jsonobj['list']:
                        code=str(e1).split('=')[0]
                        code=getNameByType(3,int(code))
                        data_list_all.append(str(code))
                        type_num = len([i for i in range(len(code)) if str.upper(code[i]) != 'X'])
                        if type_num == 1:  # 任一
                            data_list1.append(str(code))
                        elif type_num == 3:  # 任三
                            data_list3.append(str(code))
                        elif type_num == 2:  # 任二
                            data_list2.append(str(code))
                        elif type_num == 4:  # 任四
                            data_list.append(str(code))
                        elif type_num == 5:  # 任五
                            data_list5.append(str(code))
